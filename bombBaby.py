...
# You're so close to destroying the LAMBCHOP doomsday device you can taste it! But in order to do
# so, you need to deploy special self-replicating bombs designed for you by the brightest
# scientists on Bunny Planet. There are two types: Mach bombs (M) and Facula bombs (F). The bombs,
# once released into the LAMBCHOP's inner workings, will automatically deploy to all the strategic
# points you've identified and destroy them at the same time. 

# But there's a few catches. First, the bombs self-replicate via one of two distinct processes: 
# Every Mach bomb retrieves a sync unit from a Facula bomb; for every Mach bomb, a Facula bomb is
# created;
...
# overload the mass capacitors and create a singularity at the heart of the space station - not
# good! 

# And finally, you were only able to smuggle one of each type of bomb - one Mach, one Facula -
# aboard the ship when you arrived, so that's all you have to start with. (Thus it may be
# impossible to deploy the bombs to destroy the LAMBCHOP, but that's not going to stop you from
# trying!) 
...
# need to pass, so the solution would be "1". However, if M = "2" and F = "4", it would not be
# possible.
...
def solution(m, f):
    ...
    while mach != facula and mach > 0 and facula > 0:
        if mach > facula:
            transformations += mach / facula
            mach = mach % facula
        else:
            transformations += facula / mach
            facula = facula % mach
    if (mach == 1 and facula == 0) or (mach == 0 and facula == 1):
        return str(transformations - 1)
    return "impossible"
...
class Test(unittest.TestCase):
    ...
    def test3(self):
        self.assertEqual(solution('21', '13'), '6')
