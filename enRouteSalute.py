...
# salute each other - one at a time - before resuming their path. A salute is five seconds long,
# so each exchange of salutes takes a full ten seconds (Commander Lambda's salute is a bit, er,
...
# the same speed either to right or to the left, according to their direction. Whenever two
...
def solution(s):
    # as soon as we see someone walking to the right, we increment by one
    left_salute = 0
    # counter
    total_salutes = 0
    for char in s:
        if char == ">":
            left_salute += 1
        elif char == "<" and left_salute >= 1:
            total_salutes += 2 * left_salute
        # either a "-" or a right saluter "<" that doesn't have a pair
        # nothing happens
